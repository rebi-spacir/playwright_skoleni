...
test("toContainText - element contains some text", async ({ page }) => {
  ...
  await expect(
    ...
    "Welcome Page Header Contain Text" //nazev assertu,ktery se pak zobrazi v playwright ui
...
test("Soft Assert Tests", async ({ page }) => {
  await page.goto("https://tredgate.com/pmtool");
  await expect
    .soft(page.locator("h3.form-title"), "nazev assertu")
    .toHaveText("PMtool Login");
  await page.locator("#username").fill("playwright_jaro24");
  await page.locator("#password").fill("Playwright!2024");
  await page.locator('[type="submit"]').click();
});
